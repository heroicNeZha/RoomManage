...
if(isset($_POST["submit"])&&$_POST["submit"]) {
    ...
    if ($gets = mysqli_fetch_assoc($res)) {
        ...
        $_SESSION["right"] = 0;
        ?>
        <script>window.location = "1_DRM_index.php";</script><?php
    }else{
    ?>
        <script>
            alert("密码错误！");
    ...
    break;
    case "student":
        $sqlLogin = "SELECT `stu_userName` FROM tbl_student WHERE `stu_userName`='$username'AND `stu_password`='$password'";
    if ($res = mysqli_query($db, $sqlLogin)) {
    if ($gets = mysqli_fetch_assoc($res)) {
    echo "succsess";
    $_SESSION["right"] = 2;
    $_SESSION["un"] = $gets['stu_userName'];
    ?>
        <script>window.location = "3_DRM_index.php?LoginName=<?php echo $gets['stu_userName']?>";</script><?php
